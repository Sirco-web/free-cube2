...
const DEFAULT_SERVER_NAME = String(process.env.SERVER_NAME || "Cubes and Caves LAN Server").trim() || "Cubes and Caves LAN Server";
...
function createDedicatedServer(serverId) {
  ...
  return {
    ...
    motd: "Dedicated Cubes and Caves world host plus LAN/WebRTC signaling.",
...
function handleSignalCreateRoom(client, payload) {
  ...
  const room = {
    code,
    hostId: client.id,
    name: normalizeString(payload.name, "Cubes and Caves LAN World", 48) || "Cubes and Caves LAN World",
...
function tickDedicatedWorlds() {
  for (const server of dedicatedServers.values()) {
    server.worldTime = (server.worldTime + 1) % 24000;
  }
}

setInterval(tickDedicatedWorlds, 1000);
...
wss.on("connection", (ws) => {
  ...
  sendJSON(ws, {
    type: "connected",
    id: client.id,
    message: "Connected to the Cubes and Caves LAN server.",
    capabilities: {
      dedicated: true,
      lanRtcSignaling: true
    },
  ...
  ws.on("error", (error) => {
    console.warn(`[lan-server] socket error for ${client.id}:`, error.message);
  });
});

wss.on("listening", () => {
  console.log(`[lan-server] Cubes and Caves server listening on ws://localhost:${PORT}`);
  console.log("[lan-server] Modes: dedicated world sync + LAN/WebRTC signaling");
});

wss.on("error", (error) => {
  console.error(`[lan-server] Failed to start on port ${PORT}: ${error.message}`);
});
